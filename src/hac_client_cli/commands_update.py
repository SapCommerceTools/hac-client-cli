...
def _html_to_rich(html: str) -> Text:
    ...
    try:
        ...
        if extension:
            ...
            if json_output:
                ...
                # Plain tabular output for piping
                for p in ext.parameters:
...
def list_patches(
    ...
    try:
        ...
        if _is_tty():
            ...
            # Plain output - show status as applied/pending
            for p in patches:
...
def run_update(
    ...
    try:
        ...
        # Get all update data to include all extension parameters
        data = client.get_update_data()
        
        # Build parameters - include ALL extension parameters with their current values
        all_parameters = {}
        for ext in data.project_datas:
            for p in ext.parameters:
                param_key = f"{ext.name}_{p.name}"
                # Use current selected value or first available value
                if p.selected_value:
                    all_parameters[param_key] = [p.selected_value]
                elif p.available_values:
                    all_parameters[param_key] = [p.available_values[0]]
        
        # Handle patches - override specific patch parameters
        if patch:
            if extension:
                patches_ext = data.get_extension(extension)
                if not patches_ext:
            ...
            if patches_ext:
                ...
                # Enable the extension's sample checkbox - this triggers running patches
                all_parameters[f"{ext_name}_sample"] = "true"
                
                # Set all patches to "no" by default
                for p in patches_ext.parameters:
                    all_parameters[f"{ext_name}_{p.name}"] = ["no"]
                
                # Enable requested patches
                for p_name in patch:
        ...
        # If not finished yet (long-running update), follow the log
...
def _follow_log(client, json_output: bool, quiet: bool):
    ...
    Args:
        client: HacClient instance
        json_output: Output as JSON
        quiet: Plain text output
    """
    last_log = ""
    ...
    try:
        while True:
            ...
            if current_log != last_log:
                if json_output:
                    # For JSON, output the full log each time
                    print(json.dumps({
                        "log": current_log,
                        "is_complete": log.is_complete,
                        "has_errors": log.has_errors,
            ...
            if time.time() - start_time > max_wait:
                err_console.print("[yellow]Warning: Timeout waiting for update to complete[/yellow]")
                break
